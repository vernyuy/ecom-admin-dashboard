...
export default function ReviewsRatingUpdateForm(props) {
  const {
    id: idProp,
    reviewsRating: reviewsRatingModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = reviewsRatingRecord
      ? { ...initialValues, ...reviewsRatingRecord }
      : initialValues;
  ...
  const [reviewsRatingRecord, setReviewsRatingRecord] = React.useState(
    reviewsRatingModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ReviewsRating, idProp)
        : reviewsRatingModelProp;
      setReviewsRatingRecord(record);
    };
    queryData();
  }, [idProp, reviewsRatingModelProp]);
  React.useEffect(resetStateValues, [reviewsRatingRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ReviewsRating.copyOf(reviewsRatingRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ReviewsRatingUpdateForm")}
      {...rest}
    >
      <TextField
        label="Review"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || reviewsRatingModelProp)}
